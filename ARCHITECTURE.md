...
3. Audio pipeline performs downmix, high-pass filter, optional RNNoise denoise with SNR hysteresis and speech-latched mode switching, anti-aliased 48 kHz to 16 kHz resampling (rubato FFT), and earshot VAD gating with a 300 ms pre-roll ring, before single-model ASR segmentation/finalization on silence.
4. Rolling ingest bounds memory for long speech: old chunk is finalized at a low-RMS cut point near the rollover boundary and a disjoint recent tail remains in the live buffer. The overlap trimmer has been removed; finalized utterances are preserved as decoded.
...
8. Thread messages use transcript + bounded in-memory thread history; transcript can be lazily restored from the starter attachment after restart (attachment size-capped).
...
- ThreadContext: transcript + bounded turn history
...
- Transcript and per-turn message sizes sent to Gemini are bounded; full-session summarization is not implemented.
- Thread history is in-memory only and bounded; multi-turn history is not rebuilt after restart.
